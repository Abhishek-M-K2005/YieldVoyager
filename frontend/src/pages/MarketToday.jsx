...
export default function MarketToday() {
  ...
  const [marketGlobals, setMarketGlobals] = useState({ total_tvl: 0, active_protocols: 0, active_chains: 0 });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // The API does not pre-sort, leading to dead chains appearing first
        const sortedChains = chainsData.sort((a, b) => (b.tvl || 0) - (a.tvl || 0));
        setTopChains(sortedChains.slice(0, 5)); // Top 5 chains
        ...
        try {
          ...
          if (vaultsRes.ok) {
            ...
            setMarketGlobals(vaultsData.globals);
          } else {
            throw new Error("Failed to fetch vaults");
          }
        } catch (e) {
          console.warn("Backend not providing vaults, using fallback mock", e);
          setVaults([
          ...
          <div className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-lg">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1">${marketGlobals.total_tvl > 0 ? (marketGlobals.total_tvl / 1e9).toFixed(2) + 'B' : '...'}</h3>
              </div>
          ...
          <div className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-lg">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1">{marketGlobals.active_chains}</h3>
              </div>
          ...
          <div className="bg-white/5 border border-white/10 p-6 rounded-2xl backdrop-blur-lg">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <h3 className="text-2xl font-bold mt-1">{marketGlobals.active_protocols}</h3>
              </div>
            ...
            <div className="flex items-center text-sm text-gray-400">
              <span>Tracked via Scheduler</span>
            </div>
          </div>
        </div>
          ...
          {vaults.map((vault, index) => (
            <div key={index} className="bg-white/5 border border-white/10 p-6 rounded-2xl hover:border-violet-500/50 transition-all cursor-pointer group">
              ...
              <div className="flex justify-between text-xs text-gray-500 border-t border-white/5 pt-4">
                ...
                <span className="capitalize">Risk: {vault.risk_level || 'Medium'}</span>
              </div>
            </div>
          ))}
        </div>
